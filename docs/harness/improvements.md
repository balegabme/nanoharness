...
- [x] A renderer module that fails to load leaves a blank window and says nothing. (fixed: src/main/window.ts — `did-fail-load`, `preload-error` and renderer console errors write a banner into the page. The main process's own stdout is discarded on Windows, so the window is the only place a launch failure can be seen.)
- [ ] `writeGuard` screens the planner's shell by pattern, so a command that builds its target at runtime, or writes through a program the patterns do not name, gets through. It is a screen for an agent that is not trying to escape, not a boundary — the sandbox in plan §16 is the real answer — src/tools/bash.ts.
