...
switching it takes effect on the next tool call rather than on the next session.
...
- **a path outside the session folder**, resolved, with symlinks and `..`
  already followed;
...
1. **the rules** (below), as a system prompt, stable across a session, so it is
   the part a provider's cache can answer;
2. **the user's own messages**, newest last, bounded and each cut to length;
3. **the action**, fenced and labelled as data.

Not the assistant's messages, and above all **not tool results**. Tool output is
the part of a conversation somebody else can write into: a file the agent read,
an MCP server's answer, a fetched page. A judge that reads it can be argued into
an approval by the very thing it is judging. Leaving it out is both the
cheaper choice and the safer one. `goalsFrom` is where that line is drawn, and
`approval.test.ts` pins it with a tool result that tries to give orders.
...
Four buckets, in precedence order. They are sentences rather than patterns,
because the judge is a language model and a list of command names is a list
somebody has to maintain, wrong the first time a project uses a tool nobody
thought of.
...
`DEFAULT_RULES` is what a fresh install runs with. A user's own rules are
**added** to each bucket, never substituted: a settings file that could drop the
...
`allow` and `deny`. There is no third, and that is the whole shape of the mode.
...
finishes nothing, and a run that finishes nothing is worth no more than one that
was never started.
...
because an agent with no route takes the other one, which is to try the same thing again by
another road.

## When the judge cannot be reached

This is the **only** time auto mode puts a question to the person, and it is not
a verdict. It is the absence of one. The approval model was unreachable: the
endpoint is down, the key is rejected, the answer never parsed.

It is fought for first. Each rung is retried up to `JUDGE_ATTEMPTS` times on a
...
Never turned into a verdict in either direction. A judge that was never reached
has said nothing, and reading silence as a yes would be the fail-open this whole
design is built to avoid. Reading it as a no would refuse work over an outage
...
and it is bounded by the retries above rather than papered over.
...
mode that turned on, prompted for everything and said nothing would be the worst
of the three outcomes: the user would believe actions were being judged when
nothing was judging them.
...
unpinned, so the next question climbs again instead of being stuck on something
that has stopped working. When every rung fails, that is an error carrying every
reason, not a shrug.
...
The judge runs on its own model at its own prices, so its tokens are **not** the
...
computed
at the approval model's own rate at the moment of the call. The window adds that
figure to the cost of the conversation rather than pricing those tokens with the
session model's facts, which would charge an approval check at the rate of the
...
says so plainly, and it should keep saying so. The action text is
attacker-influenceable, which is why it is fenced and labelled as data and why
tool results stay out of the prompt. But a prompt boundary is only a
mitigation.
