...
workspace configures, and their tools sit in the same list as `read` and
`bash` — the model cannot tell which is which, and does not need to. Plan §7.
...
A **protocol error** — a `-32700` through `-32603`, a malformed envelope, an
unknown method — is a bug in this client or that server. The model never sees
...
Cancellation is advisory in MCP — the server may finish the work anyway — and
the point is that this side stops waiting, not that the work stops.
...
broken. The drain matters too — an unread pipe fills and blocks the child.
...
handshake settled on — a server that receives no version header is entitled to
assume an older protocol and answer in a shape this client stopped expecting.
...
member — an argument that may also be null is still, to the model typing it, a
string — and `integer` becomes `number`, which is the only thing the provider
layer knows.
...
the configured server name rather than generated, so the tool definitions —
...
unique, and derived rather than counted, so it is the same bytes every time.
...
merging field by field — a half-overridden command line is a server nobody
configured — and `"enabled": false` is how a project switches a global server
off without editing the file every other workspace reads.

Secret-free by schema (plan §16): a token is *named*, never written. There is
no field a key could land in, which is what keeps the file diffable and safe to
paste into an issue. stdio servers get named variables passed through from the
harness's environment; HTTP servers get a bearer token read from the variable
`tokenEnv` names.
...
The system prompt carries the servers this session actually connected to, the
two file paths, and the shape of an entry. Everyone gets that much: it is what
"what tools do you have" needs, and it is not a licence to edit anything.
...
user meant. The first version told both, and a builder read the rule that said
*delegate* and the command that said *here is how*, weighed them against each
other in its own thinking, and did the work itself. A prompt that argues with
...
tools it has answers from its training set when the prompt says nothing — and
one asked to add an MCP server invented a policy forbidding it, reading the
prompt's "do not install anything" as covering a JSON file in the workspace. A
server is configuration, not an install, and the MCP block says so in as many
words to the one role that does the configuring.
...
One hub per session, connected before the first request — the tool definitions
have to be in front of the first turn, and a tool discovered later would move
bytes the provider has already cached. Servers are connected one after another
rather than all at once: an `npx -y` on a cold cache downloads a package, and
four of those at once on a laptop is worse than four in a row.
...
name — starting silently with no MCP tools after a stray comma looks exactly
like a harness that never supported them.

A server that fails *after* it started — a catalog that errors, a handshake that
times out — is closed on the way out of that failure, because a client that
never reached the hub's list is a client nothing else will ever close.
