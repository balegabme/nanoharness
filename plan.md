...
**Tool naming:** MCP tools are exposed to models as `mcp__<server>__<tool>`, so no collisions
...
**Design tokens (initial values; live in `docs/harness/ui.md` as CSS variables, ours and tunable):**
...
6. Slash commands + project memory: `.nanoharness/commands/*.md`; project memory file,
   list-mode injected.
7. Prompt snippets (§9).
8. Keyboard-first UI and command palette.
...
- Deprioritized, on counter-evidence: terse-output prompting: 65% headline vs ~8.5% on
