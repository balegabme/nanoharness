...
export function buildSystemPrompt(env: PromptEnvironment): string {
  ...
  lines.push(
    ...
    '- Read a file before you edit it. Use `edit` for a change to an existing file and `write` to create one or replace all of it. Each says what changed, so do not read the file back to check.',
